...
class User {
    ...
    public function search_users($query, $gender, $relationship, $status) {
        ...
        /* get users */
        if($query) {
            $get_users = $db->query(sprintf('SELECT * FROM users WHERE (user_name LIKE %1$s OR user_firstname LIKE %1$s OR user_lastname LIKE %1$s OR CONCAT(user_firstname,  " ", user_lastname) LIKE %1$s)'.$where.' ORDER BY user_firstname ASC LIMIT %2$s', secure($query, 'search'), secure($system['max_results'], 'int', false) )) or _error(SQL_ERROR_THROWEN);
        } else {
            $get_users = $db->query(sprintf("SELECT * FROM users WHERE 1 = 1 ".$where." ORDER BY user_firstname ASC LIMIT %s", secure($system['max_results'], 'int', false) )) or _error(SQL_ERROR_THROWEN);
        }
        if($get_users->num_rows > 0) {
            while($user = $get_users->fetch_assoc()) {
                $user['user_picture'] = get_picture($user['user_picture'], $user['user_gender']);
    ...
    public function get_stories() {
        ...
        if($get_stories->num_rows > 0) {
            while($story = $get_stories->fetch_assoc()) {
                $story['user_picture'] = get_picture($story['user_picture'], $story['user_gender']);
                /* get story media */
    ...
    public function get_forum_thread($thread_id, $update_views = false) {
        ...
        /* update thread views */
        if($update_views) {
            $db->query(sprintf("UPDATE forums_threads SET views = views + 1 WHERE thread_id = %s", secure($thread['forum_id'], 'int'))) or _error(SQL_ERROR_THROWEN);
        }
        return $thread;
    }
    ...
    public function edit_campaign($campaign_id, $args) {
        ...
        if($remining > $campaign['user_wallet_balance']) {
            throw new Exception(__("The remining spend is larger than current wallet credit")." <strong>".$system['system_currency_symbol'].$campaign['user_wallet_balance']."</strong> ".__("You need to")." ".__("Replenish wallet credit"));
        }
        /* validate campaign bidding */
        if(!in_array($args['campaign_bidding'], array('click','view'))) {
            throw new Exception(__("You have to select a valid bidding"));
        ...
        if($args['campaign_bidding'] == "click") {
            if($remining < $system['ads_cost_click']) {
                throw new Exception(__("The cost per click is larger than your campaign remining spend")." <strong>".$system['system_currency_symbol'].$remining."</strong> ".__("increase the campaign budget to resume the campaign"));
            }
        }
        if($args['campaign_bidding'] == "view") {
            if($remining < $system['ads_cost_view']) {
                throw new Exception(__("The cost per view is larger than your campaign remining spend")." <strong>".$system['system_currency_symbol'].$remining."</strong> ".__("increase the campaign budget to resume the campaign"));
            }
        }
        /* validate audience countries */
        $args['audience_countries'] = (isset($args['audience_countries']) && is_array($args['audience_countries']))? implode(',',$args['audience_countries']) : "";
    ...
    public function wallet_transfer($user_id, $amount) {
        ...
        $db->query(sprintf("UPDATE users SET user_wallet_balance = IF(user_wallet_balance=0,0,user_wallet_balance-%s) WHERE user_id = %s", secure($amount), secure($this->_data['user_id'], 'int') )) or _error(SQL_ERROR_THROWEN);
    ...
    /**
     * wallet_withdraw_affiliate
     * 
     * @param integer $amount
     * @return void
     */
    public function wallet_withdraw_affiliate($amount) {
        global $db, $system;
        /* check if ads enabled */
        if(!$system['ads_enabled']) {
            throw new Exception(__("This feature has been disabled by the admin"));
        ...
        /* validate amount */
        if(is_empty($amount) || !is_numeric($amount)) {
            throw new Exception(__("You must enter valid amount of money"));
        }
        /* check viewer balance */
        ...
        $db->query(sprintf("UPDATE users SET user_affiliate_balance = IF(user_affiliate_balance=0,0,user_affiliate_balance-%s) WHERE user_id = %s", secure($amount), secure($this->_data['user_id'], 'int') )) or _error(SQL_ERROR_THROWEN);
        ...
        /* wallet transaction */
        $this->wallet_set_transaction($this->_data['user_id'], 'withdraw', 0, $amount, 'in');
        $_SESSION['wallet_withdraw_amount'] = $amount;
    }
        ...
        switch ($edit) {
            ...
            case 'phone':
                ...
                if($args['phone'] != $this->_data['user_phone']) {
                    ...
                    /* update user */
                    $db->query(sprintf("UPDATE users SET user_phone = %s, user_activation_key = %s, user_activated = '0' WHERE user_id = %s", secure($args['phone']), secure($activation_key), secure($this->_data['user_id'], 'int') )) or _error(SQL_ERROR_THROWEN);
                    // send activation key
