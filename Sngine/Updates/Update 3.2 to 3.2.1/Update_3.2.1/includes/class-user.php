...
class User {
    ...
    public function __construct() {
        ...
        if(isset($_COOKIE[$this->_cookie_user_id]) && isset($_COOKIE[$this->_cookie_user_token])) {
            ...
            if($get_user->num_rows > 0) {
                ...
                /* prepare full name */
                $this->_data['name'] = ($system['show_usernames_enabled'])? $this->_data['user_name']: $this->_data['user_firstname']." ".$this->_data['user_lastname'];
                /* check unusual login */
                ...
                if($system['funding_enabled']) {
                    $this->_data['can_raise_funding'] = $this->check_module_permission($system['funding_permission']);
                }
                ...
                if($system['stories_enabled']) {
                    $this->_data['can_add_stories'] = $this->check_module_permission($system['stories_permission']);
                }
                /* check invitation permission */
                if($system['invitation_enabled']) {
                    $this->_data['can_invite_users'] = $this->check_module_permission($system['invitation_permission']);
                }
                /* check audio call permission */
                if($system['audio_call_enabled']) {
                    $this->_data['can_start_audio_call'] = $this->check_module_permission($system['audio_call_permission']);
                }
                /* check video call permission */
                if($system['video_call_enabled']) {
                    $this->_data['can_start_video_call'] = $this->check_module_permission($system['video_call_permission']);
                }
    ...
    public function get_gender($gender_id) {
        ...
        return $get_gender->fetch_assoc()['gender_name'];
    }
    ...
    public function get_new_people($offset = 0, $random = false) {
        ...
        if($system['location_finder_enabled']) {
            ...
            if($random) {
                $get_users = $db->query(sprintf("SELECT user_id, user_name, user_firstname, user_lastname, user_gender, user_picture, user_subscribed, user_verified FROM users ".$where." ORDER BY RAND() LIMIT %s, %s", secure($offset, 'int', false), secure($system['min_results'], 'int', false) )) or _error("SQL_ERROR_THROWEN");
            } else {
    ...
    public function search($query) {
        ...
        /* search users */
        $get_users = $db->query(sprintf('SELECT user_id, user_name, user_firstname, user_lastname, user_gender, user_picture, user_subscribed, user_verified FROM users WHERE user_banned = "0" AND (user_name LIKE %1$s OR user_firstname LIKE %1$s OR user_lastname LIKE %1$s OR CONCAT(user_firstname,  " ", user_lastname) LIKE %1$s) ORDER BY user_firstname ASC LIMIT %2$s, %3$s', secure($query, 'search'), secure($offset, 'int', false), secure($system['max_results'], 'int', false) )) or _error("SQL_ERROR_THROWEN");
        if($get_users->num_rows > 0) {
            while($user = $get_users->fetch_assoc()) {
                $user['user_picture'] = get_picture($user['user_picture'], $user['user_gender']);
                /* get the connection between the viewer & the target */
    ...
    public function search_users($distance, $keyword, $gender, $relationship, $status) {
        ...
        if(isset($relationship) && !in_array($relationship, array('any','single', 'relationship', 'married', "complicated", 'separated', 'divorced', 'widowed'))) {
            return $results;
        }
        /* validate status */
        ...
        if($system['location_finder_enabled']) {
            ...
            /* prepare query */
            $query = sprintf("SELECT user_id, user_name, user_firstname, user_lastname, user_gender, user_picture, user_subscribed, user_verified, (%s * acos(cos(radians(%s)) * cos(radians(user_latitude)) * cos(radians(user_longitude) - radians(%s)) + sin(radians(%s)) * sin(radians(user_latitude))) ) AS distance FROM users ", secure($unit, 'int'), secure($this->_data['user_latitude']), secure($this->_data['user_longitude']), secure($this->_data['user_latitude']));
            $query .= $where;
            $query .= sprintf(" HAVING distance < %s ORDER BY distance ASC LIMIT %s", secure($distance, 'int'), secure($system['max_results'], 'int', false) );
        } else {
            /* prepare query */
            $query = "SELECT user_id, user_name, user_firstname, user_lastname, user_gender, user_picture, user_subscribed, user_verified FROM users ";
            $query .= $where;
            ...
            while($notification = $get_notifications->fetch_assoc()) {
                ...
                switch ($notification['action']) {
                    ...
                    case 'funding_donation':
                        $notification['icon'] = "fa fa-hand-holding-usd";
                        $notification['url'] = $system['system_url'].'/posts/'.$notification['node_url'];
                        $notification['message'] = __("donated to your funding request with")." ".$system['system_currency_symbol'].$notification['node_type'];
                        break;
    ...
    public function post_notification($args = []) {
        ...
        if($system['onesignal_notification_enabled'] && $receiver['onesignal_user_id']) {
            ...
            if($receiver['user_language'] != DEFAULT_LOCALE) {
                $t->loadTranslations(Gettext\Translations::fromPoFile(ABSPATH.'content/languages/locale/'.$receiver['user_language'].'/LC_MESSAGES/messages.po'));
            }
            /* prepare notification notify_id */
            $notify_id = ($notify_id)? "?notify_id=".$notify_id : "";
            /* prepare notification node_url */
            ...
            switch ($action) {
                ...
                case 'funding_donation':
                    $notification['url'] = $system['system_url'].'/posts/'.$node_url;
                    $notification['message'] = __("donated to your funding request with")." ".$system['system_currency_symbol'].$node_type;
                    break;
    ...
    public function notify_system_admins($action, $notify_moderators = false) {
        ...
        while($system_admin = $get_system_admins->fetch_assoc()) {
            /* post notification */
            $this->post_notification( ['to_user_id' => $system_admin['user_id'], 'action' => $action] );
        }
    }
    ...
    public function check_new_calls($type) {
        ...
        $call['caller_name'] = ($system['show_usernames_enabled'])? $call['user_name']: $call['user_firstname']." ".$call['user_lastname'];
    ...
    public function publisher($args = []) {
        ...
        $post['post_author_name'] = $this->_data['name'];
        ...
        } elseif($args['photos'] && count($args['photos']) > 0) {
    ...
    public function scraper($url) {
        ...
        if($embed) {
            ...
            if( $embed->type == "link" || ($embed->type == "rich" && !in_array($embed->providerName, ["Twitter", "SoundCloud"])) ) {
                $return['source_type'] = "link";
                $return['source_host'] = $url_parsed['host'];
            } else {
                if($embed->providerName == "Twitch") {
                    $embed->code = str_replace("meta.tag", $_SERVER['HTTP_HOST'], $embed->code);
                }
    ...
    public function share($post_id, $args) {
        ...
        switch ($args['share_to']) {
            ...
            case 'group':
                ...
                if(!$group['group_publish_enabled']) {
                ...
                if(!$group_approved) {
                    /* send notification to group admin */
                    $this->post_notification( array('to_user_id'=>$group['group_admin'], 'action'=>'group_post_pending', 'node_type'=>$group['group_title'], 'node_url'=>$group['group_name']."-[guid=]".$post_id) );
                }
                break;
     ...
     * @return array
     */
    public function get_comments($node_id, $offset = 0, $is_post = true, $pass_privacy_check = true, $post = [], $top_sorted = false, $last_comment_id = null) {
        global $db, $system;
        $comments = [];
        $offset *= $system['min_results'];
    ...
    public function comment($handle, $node_id, $message, $photo, $voice_note) {
        ...
        if(!$post['is_page_admin']) {
            ...
            $comment['author_name'] = $this->_data['name'];
        ...
        /* post notification */
        if($handle == "comment") {
            $this->post_notification( array('to_user_id'=>$parent_comment['author_id'], 'from_user_id'=>$comment['user_id'], 'from_user_type'=>$comment['user_type'], 'action'=>'reply', 'node_type'=>$parent_comment['node_type'], 'node_url'=>$parent_comment['node_id'], 'notify_id'=>'comment_'.$comment['comment_id']) );
    ...
    public function campaign_potential_reach($countries = [], $gender = 'all', $relationship = 'all') {
        ...
        /* validate relationship */
        if(isset($relationship) && !in_array($relationship, array('all','single', 'relationship', 'married', "complicated", 'separated', 'divorced', 'widowed'))) {
            return $results;
        }
        /* prepare where statement */
        $where = "";
    ...
    public function wallet_withdraw_funding($amount) {
        ...
        if($this->_data['user_funding_balance'] < $amount) {
            throw new Exception(__("The amount is larger than your current funding balance")." <strong>".$system['system_currency_symbol'].$this->_data['user_funding_balance']."</strong>");
        }
     ...
     * @return string
     */
    public function coinpayments($handle, $price, $id = null, $package_name = null) {
        global $system;
        /* prepare */
        switch ($handle) {
            case 'packages':
                ...
                $URL['ipn'] = $system['system_url']."/includes/ajax/payments/coinpayments_webhook.php?handle=packages&package_id=".$id;
                break;
            ...
            case 'donate':
                $product = __('Funding Donation');
                $description = __('Pay For')." ".$system['system_title'];
                $URL['success'] = $system['system_url']."/settings/coinpayments";
                $URL['cancel'] = $system['system_url'];
                $URL['ipn'] = $system['system_url']."/includes/ajax/payments/coinpayments_webhook.php?handle=donate&post_id=".$id;
                break;
     ...
     * @return array
     */
    public function get_fundings($args = []) {
        global $db, $system;
        /* initialize arguments */
        $offset = !isset($args['offset']) ? 0 : $args['offset'];
        $results = !isset($args['results']) ? $system['max_results_even'] : $args['results'];
    ...
    public function get_invitation_codes_details() {
        global $db;
        $codes = [];
        $get_codes = $db->query(sprintf('SELECT invitation_codes.*, users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture FROM invitation_codes LEFT JOIN users ON invitation_codes.used_by = users.user_id WHERE invitation_codes.created_by = %s', secure($this->_data['user_id'], 'int'))) or _error("SQL_ERROR");
    ...
    public function send_invitation_email($email, $code) {
        ...
        $subject = $this->_data['name']." ".__("Invite you to join")." ".$system['system_title'];
    ...
    public function send_invitation_sms($phone, $code) {
        ...
        $message = $this->_data['name']." ".__("Invite you to join")." ".$system['system_title']." ".$system['system_url']."/?ref=".$this->_data['user_name']."&invitation_code=".$code;
        /* send SMS */
    ...
    public function settings($edit, $args) {
        ...
        switch ($edit) {
            ...
            case 'basic':
                ...
                /* validate gender */
                if(!$this->check_gender($args['gender'])) {
                ...
                switch ($system['two_factor_type']) {
                    ...
                    case 'google':
                        if(isset($args['gcode'])) {
                            ...
                            require_once(ABSPATH.'includes/libs/GoogleAuthenticator/GoogleAuthenticatorInterface.php');
                            require_once(ABSPATH.'includes/libs/GoogleAuthenticator/GoogleAuthenticator.php');
                            require_once(ABSPATH.'includes/libs/GoogleAuthenticator/FixedBitNotation.php');
            ...
            case 'privacy':
                ...
                $args['user_privacy_newsletter'] = (isset($args['user_privacy_newsletter']))? '1' : '0';
                ...
                $db->query(sprintf("UPDATE users SET 
                    ...
                    secure($args['user_privacy_newsletter']), 
