...
try {
	...
	switch ($_GET['do']) {
		case 'edit_account':
			...
			/* update */
			$db->query(sprintf("UPDATE users SET user_verified = %s, user_banned = %s, user_activated = %s, user_group = %s, user_name = %s, user_email = %s, user_phone = %s, user_password = %s WHERE user_id = %s", secure($_POST['user_verified']), secure($_POST['user_banned']), secure($_POST['user_activated']), secure($_POST['user_group'], 'int'), secure($_POST['user_name']), secure($_POST['user_email']), secure($_POST['user_phone']), secure($_POST['user_password']), secure($_GET['id'], 'int') )) or _error(SQL_ERROR_THROWEN);
			/* return */
