...
class User {
    ...
    public function get_pro_members() {
        global $db, $system;
        $pro_members = array();
        $get_pro_members = $db->query(sprintf("SELECT * FROM users WHERE user_subscribed = '1' ORDER BY RAND() LIMIT %s", $system['max_results'])) or _error(SQL_ERROR_THROWEN);
    ...
    public function friendship_declined($user_id) {
        ...
        if($this->_logged_in) {
            $check = $db->query(sprintf('SELECT * FROM friends WHERE status = -1 AND (user_one_id = %1$s AND user_two_id = %2$s) OR (user_one_id = %2$s AND user_two_id = %1$s)', secure($this->_data['user_id'], 'int'),  secure($user_id, 'int') )) or _error(SQL_ERROR_THROWEN);
            if($check->num_rows > 0) {
                return true;
            }
        }
    ...
    public function share($post_id, $args) {
        ...
        switch ($args['share_to']) {
            ...
            case 'page':
                /* check if the page is valid */
                $check_page = $db->query(sprintf("SELECT * FROM pages WHERE page_id = %s", secure($args['page'], 'int') )) or _error(SQL_ERROR_THROWEN);
                if($check_page->num_rows == 0) {
                    _error(400);
                }
                $_page = $check_page->fetch_assoc();
            ...
            case 'group':
                /* check if the group is valid & the viewer is group member (approved) */
                $check_group = $db->query(sprintf("SELECT groups.* FROM groups INNER JOIN groups_members ON groups.group_id = groups_members.group_id WHERE groups_members.approved = '1' AND groups_members.user_id = %s AND groups_members.group_id = %s", secure($this->_data['user_id'], 'int'), secure($args['group'], 'int') )) or _error(SQL_ERROR_THROWEN);
                $_group = $check_group->fetch_assoc();
                /* insert the new shared post */
